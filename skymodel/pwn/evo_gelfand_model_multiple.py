...
def launch_evo_m(pwn_n, age, l0, e0, mej2, nn, eta, t0, eps, dens, Tfir, Ufir, Tnir, Unir, binss, tmin, ebreak, alpha1, alpha2, dist): #function for launch all the computation (more info in every step of the computation)
    ...
    if age > 2000 and t0 < 500 and l0 < 1e38: #This is needed since for some set of parameters the bohm diffusion breaks the GAMERA computation (need to be checked!). In such cases we neglect it for the moment. 
        no_escape = True
    else:
        no_escape = False
    R, V, V_ej, LT, B, M, EMAX, SED, P, F, E, DENS_EJ = Evo_pwn(t, l0, e0, mej2, nn, eta, t0, dens, Tfir, Ufir, Tnir, Unir, R_rs, v_rs, R_snr, v_snr, eps, ebreak, alpha1, alpha2, no_escape)
    if R_rs[len(R)-1] <= R[-1]:
        sed = 0
        tot = 0
    else:
...
def Evo_pwn(t, l0, e0, mej2, nn, eta, t0, dens, Tfir, Ufir, Tnir, Unir, R_rs, v_rs, R_snr, v_snr, eps, ebreak, alpha1, alpha2, no_escape):
    """
