...
SPECTRUM_NODE_TEMPLATE_READABLE = """\
    ...
    <parameter name="Intensity" value="{dnde:.5g}" scale="1e-10" min="1e-12" max="1e1" free="1"/>
    </node>
"""
...
for file in glob.glob(FOLDER_FOR_RESULTS+"*.ecsv"):
    ...
    NUMBER=np.append(NUMBER,number)

NUMBER=remove_value_from_array(NUMBER, 1)
...
if __name__ == '__main__':
    for version in NUMBER:  #range(0,100):
        ...
        filename = FOLDER_FOR_RESULTS+'/ctadc_skymodel_gps_sources_pevatron_{}_keep.ecsv'.format(version)
        print('Writing {}'.format(filename))
        table.write(filename, format='ascii.ecsv', overwrite=True)

        filename = FOLDER_FOR_RESULTS+'/ctadc_skymodel_gps_sources_pevatron_{}.xml'.format(version)
        print('Writing {}'.format(filename))
